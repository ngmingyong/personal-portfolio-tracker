...
function AddDividendDialog({ positionIdWithAddDividendDialogOpened, stockCodeWithAddDividendDialogOpened, closeDialog, apiCall, endpoint, method }) {
  ...
  const handleChange = (e) => {
    ...
    if ((name === 'dividendPerShare') &&  !(/^(0|[1-9]\d*)(\.\d{1,4})?$|^$/.test(value))){
      alert('Dividend per share cannot be negative, and must be either an integer or a decimal with up to four decimal places.');
      return;
    }
